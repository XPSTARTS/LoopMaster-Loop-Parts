...
class YouTubeLooper {
  ...
  private createButton(text: string, color: string, title: string, size: string, fontSize: string, padding: string): HTMLButtonElement {
  ...
  private updateProgressHighlight(): void {
    ...
    if (widthPercent <= 0 || !isFinite(widthPercent)) return;
